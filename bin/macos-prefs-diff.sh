...
# macos-prefs-diff.sh
#   from paulirish/dotfiles (bin/macos-prefs-diff.sh), adapted for this repo's
#   .osx file and made self-locating instead of hardcoding a clone path.
#
# Diffs the live `defaults` state on this machine against every
# `defaults write ...` line in .osx, to catch drift between what the script
# claims to set and what's actually configured. Doesn't change anything.

# set -euxo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd -P)"
SETTINGS_FILE="${MACOS_PREFS_FILE:-$SCRIPT_DIR/../.osx}"
TEMP_DOMAIN="com.dotfiles.macos-prefs-diff.temp"
...
while IFS= read -r line || [ -n "$line" ]; do
    ...
    if [[ "$current_cmd" =~ ^(sudo[[:space:]]+)?defaults[[:space:]]+write ]]; then
        ...
        [[ "${cmd_parts[0]}" == "sudo" ]] && start_idx=3
                ...
                # Skip Dock persistent items as they are noisy and rebuilt via functions in .osx
